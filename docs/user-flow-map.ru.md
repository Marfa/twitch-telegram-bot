...
flowchart TD
  ...
  ADV -->|да| OPT["Дополнительно: toggles"]
  ADV -->|нет| SKIP["Пропуск ignore/delay/repeat/delete"]
  OPT --> IGN["Игнор-слова (если отмечено)"]
  IGN --> LP["Превью ссылок?"]
  OPT -->|без ignore| LP
  LP -->|upcoming| SRM["Минуты до старта"]
  LP -->|live/cat/end| DL["Задержка (если отмечено)"]
  DL -->|live| RP["Повторы (если отмечено)"]
  DL -->|cat/end| DEST
  RP --> DEST["Куда слать"]
  SRM --> DEST
  SKIP --> DEST
  DEST -->|личка| DONE["Готово"]
  DEST -->|канал или группа| DEL["Удалять старые? (если отмечено)"]
  DEL --> DONE
```
...
Не-live типы и шаги delay/ignore/repeat/delete — через Premium / gate. Простой режим (продвинутый выкл) пропускает ignore / delay / repeat / delete / кнопку чата. В продвинутом режиме после картинки — экран «Дополнительно» с тумблерами (включая кнопку чата); неотмеченные шаги пропускаются.
...
### 2.6 Дополнительно (продвинутый режим)

| | |
|---|---|
| Текст | Отметьте нужные опции + краткие описания видимых тумблеров |
| Inline | `⬜️/✅ Игнор` · `⬜️/✅ Задержка` · `⬜️/✅ Повторы` (live) · `⬜️/✅ Удалять предыдущие` · `⬜️/✅ Кнопка чата` · `Далее` |
| Далее | Только отмеченные шаги ниже; кнопка чата применяется без отдельного yes/no |
