...
`Мне повезёт`: индикатор «Ищем стримы…»; 5 игр IGDB random → live язык бота / любой; затем 5 **случайных** из recently released → live; если пусто — VOD по категориям обоих наборов (язык бота / любой); 18+ включены. Админу в результате — список игр Random / Recently released.
