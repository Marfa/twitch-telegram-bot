...
Дата обновления карты: **2026-08-27**.
...
Deep link `?start=share_{token}` (🧪 бета **Поделиться оповещением**): в списке кнопка рядом с паузой; по нажатию бот присылает пояснение и **ссылку для друга**. После обычного онбординга у друга — предложение **добавить оповещение** (`✅ Добавить` / `Нет, спасибо`). Назначение у клона — всегда ЛС получателя; premium-поля сохраняются, у free при открытии редактирования сбрасываются в дефолт.
...
| Reply | Далее |
|---|---|
| `📋 Мои подписки` | Выбор типа (если несколько) → список с `⏸ Выкл` / `✅ Вкл` и (🧪 бета) **Поделиться** в одном ряду; при длинном тексте — страницы `‹ n/m ›` |
| `✏️ Редактировать` | Тип → выбор (страницы кнопок при длинном списке) → «Что изменить?»; у free без продвинутого режима advanced-поля сбрасываются при входе |
| `🗑 Удалить подписку` | Тип → (опционально `🧺 Корзина`) → мультивыбор со страницами → удалить |
...
Inline: отметки `✅ 🗑 #N …` · `🗑 Удалить все` · `🗑 Удалить выбранные ({count})` · `Сбросить` · `🧺 Корзина` · подтверждение «Удалить все» (`Да` / `Нет`)

Корзина (удалённые): при нескольких типах — выбор типа → список по последним `10/30` дням (free/premium) → мультивыбор → `♻️ Восстановить выбранные`. 30 дней — Premium / à la carte.
