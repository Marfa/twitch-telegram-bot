...
Дата обновления карты: **2026-08-31**.
...
Deep link `?start=share_{token}` (🧪 бета **Поделиться оповещением**): в списке у подписки кнопка `📤`; по нажатию бот присылает пояснение и **ссылку для друга**. После обычного онбординга у друга — предложение **добавить оповещение** (`✅ Добавить` / `Нет, спасибо`). Назначение у клона — всегда ЛС получателя; premium-поля сохраняются, у free при открытии редактирования сбрасываются в дефолт.
