...
Deep link `?start=share_{token}` (🧪 бета **Поделиться оповещением**): после обычного онбординга (язык → welcome → демо при первом старте) бот предлагает **добавить оповещение** с теми же настройками (`✅ Добавить` / `Нет, спасибо`). Назначение у клона — всегда ЛС получателя; premium-поля сохраняются, у free при открытии редактирования сбрасываются в дефолт.
