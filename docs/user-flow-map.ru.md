...
Deep link `?start=share_{token}` (🧪 бета **Поделиться оповещением**): в списке под именем — текст + кнопка; по нажатию бот присылает пояснение и **ссылку для друга**. После обычного онбординга у друга — предложение **добавить оповещение** (`✅ Добавить` / `Нет, спасибо`). Назначение у клона — всегда ЛС получателя; premium-поля сохраняются, у free при открытии редактирования сбрасываются в дефолт.
...
| Reply | Далее |
|---|---|
| `📋 Мои подписки` | Выбор типа (если несколько) → список с `⏸ Выкл` / `✅ Вкл`; при длинном тексте — страницы `‹ n/m ›`; под именем — ссылка **Поделиться оповещением** (🧪 бета) |
