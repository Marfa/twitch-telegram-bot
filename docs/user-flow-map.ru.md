...
`Мне повезёт`: индикатор «Ищем стримы…»; 5 игр IGDB random → live язык бота / любой; затем 5 recently released → live; если пусто — VOD по тем же категориям; 18+ включены. Админу в результате — список игр Random / Recently released.
