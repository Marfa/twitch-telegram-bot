...
| `⏸ Приостановить оповещения` | Бета `pause-notifications`. Текст про период в днях → ввод числа (0 = снова включить) · Reply `Отмена` |
...
Inline: отметки `✅ 🗑 #N …` · `🗑 Удалить выбранные ({count})` · `Сбросить выбор` · `🧺 Корзина`

Корзина (удалённые, бета `deleted-subscriptions-cart`): список по последним `10/30` дням (free/premium) → мультивыбор → `♻️ Восстановить выбранные`. Включение: **Настройки → 🧪 Бета-режим**.
...
| (Неделя) Игра на день | Текст игры · `Стрим не планируется` · `Завершить создание расписания` · `Отмена` |
| (Неделя) Время | `ЧЧ:ММ` |
