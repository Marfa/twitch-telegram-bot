...
`Мне повезёт`: индикатор «Ищем стримы…»; 5 игр IGDB random → live язык бота / любой; затем 5 **случайных** из recently released → live; если пусто — VOD по категориям обоих наборов (язык бота / любой); 18+ включены.
...
| `💬 Оповещения об ЛС` | Описание + чекбокс «Включить»; OAuth Twitch; входящие whisper в Telegram |
| `🎛 Продвинутый режим` | Вкл/выкл (Premium) |
| `🧪 Бета-режим` | Список бета-функций: присоединиться / GitHub Issue |
...
Текст: заголовок + «При включении этой опции вы будете получать оповещения о новых личных сообщениях на Twitch.»

Inline: `⬜️/✅ Включить`. Первое включение — OAuth Twitch (`user:read:whispers`). Входящее whisper: отправитель, текст, ссылка на переписку.
