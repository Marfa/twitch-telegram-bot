...
4. **Дополнительно** (только **продвинутый режим**) — чеклист с краткими описаниями: игнор ключевых слов, отложенная отправка, заглушка повторов (начало стрима), удалять предыдущие в канале/группе, кнопка чата; неотмеченные шаги пропускаются
5. Превью ссылок (шаг пропускается, если есть картинка)
6. Отложить отправку (минуты) — если отмечено в «Дополнительно»; после старта / смены категории / офлайна, перед отправкой Helix перепроверяется
...
Пока продвинутый режим выкл, шаги 4 / 6 / 7 / 10 пропускаются (кнопка чата тоже); на первом шаге мастера — пометка про упрощённый режим. Вкл/выкл: **⚙️ Настройки → 🎛 Продвинутый режим** (Premium).
