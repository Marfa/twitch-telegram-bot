...
- синхронизация — период в днях, refresh token хранится зашифрованно; раз в период добавляются новые фолловы и удаляются отфолловленные импорты (ручные подписки не трогаются);
- оповещения создаются **на паузе** (DM себе); в Настройках — **Синхронизация подписок** (период / отключить).
