...
**Редактирование подписки** — в том же порядке, что и при создании: шаблон (с чекбоксом **Очистка названия**), картинка, ключевые слова / задержка / повторы / удаление (пункты меню только в **продвинутом режиме**), превью ссылок (скрыто, если есть картинка), напоминания по расписанию (если включали при создании), куда слать. Для оповещений о **смене категории стрима** при включённом удалении — отдельно «удалять и другие оповещения». В конце меню: **Сменить тип**, **Скопировать**, **Скопировать и сменить тип** (при отмене выбора типа копия не создаётся).
