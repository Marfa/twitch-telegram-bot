...
| Текст | Шаблон с плейсхолдерами; примеры `{username}`, `{game}`, `{name}` — [полный список](https://bot.themarfa.name/placeholders?lang=ru). В редакторе: **Вкл / Выкл** — очистка `{name}` от `@стримеров` (если канал есть на Twitch) и `!команд` (по умолчанию выкл) |
...
**Шаблон сообщения** — в мастере показаны примеры `{username}`, `{game}`, `{name}`. Полный список плейсхолдеров (в т.ч. `started_at`, `viewer_count`, `thumbnail_url`, `tags`, …): [`PUBLIC_BASE_URL/placeholders`](https://bot.themarfa.name/placeholders?lang=ru) (прод: `https://bot.themarfa.name`). Кнопки **Вкл / Выкл** в редакторе шаблона (создание и правка) включают очистку названия стрима `{name}`: убираются упоминания `@username`, если такой стример есть на Twitch, и команды вида `!команда`. По умолчанию выкл; нажатие не закрывает редактор.
...
**Редактирование подписки** — в том же порядке, что и при создании: шаблон (с **Вкл / Выкл** очистки `{name}`), картинка, ключевые слова, превью ссылок (скрыто, если есть картинка), задержка, повторы (не для смены категории и окончания), напоминания по расписанию (если включали при создании), куда слать, удаление старых сообщений. Для **смены категории** при включённом удалении — отдельно «удалять и другие оповещения».
