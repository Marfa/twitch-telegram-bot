...
| 🎲 Оповещение по игре | В **➕ Новая подписка** (под Drops): поиск стримов или сразу создать оповещение по категории; live → иначе VOD; фильтры в редакторе; в **📋 Мои подписки** — отдельная категория |
...
| Режим | Что делает |
|---|---|
| Поиск стримов по игре | Категория → подборка live (если нет — недавние VOD); оповещение не создаётся |
| Оповещения по игре | Категория → сразу создаёт оповещение + та же подборка |

Созданные оповещения — в **📋 Мои подписки** в категории «Оповещение по игре». Теги / зрители / язык / исключить 18+ / частота (минуты, по умолчанию 60) — в **✏️** редакторе.

После подборки: **Ещё варианты** или снова поиск / создание.
