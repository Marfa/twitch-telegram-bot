...
| 🎁 Раздачи игр | Бета: **➕ Новая подписка** → под «Выход релизов»; магазины + платформы (GamerPower + IsThereAnyDeal); карточки по 5 (фото+текст, магазин / стримы, «Показать ещё»); о новых — короткая сводка + «Подробности» |
