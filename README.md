...
| 📅 Выход релизов | Бета: **➕ Новая подписка** → под «Оповещение по игре»; поиск в локальном IGDB; даты/платформы (если даты нет — создаём всё равно, подтянем позже); за N дней; общий лимит 5 активных; пауза когда все выбранные платформы вышли |
| 🎲 Что посмотреть? | В **📦 Прочее**: сразу «Мне повезёт» |
| Картинка | Опционально к уведомлению: превью стрима (кадр, обновление 30 мин), видео-превью ⭐ (GIF ~30 с, обновление 30 мин), обложка игры или своё фото — в начале или в конце подписи; превью ссылок тогда выкл |
...
При первом `/start` бот предложит выбрать язык (русский или English), затем покажет приветствие и **главное меню**. При первом запуске (не share) для **ru** также создаётся welcome-оповещение на случайный Премиум-канал; для **en** канал `PREMIUM_TWITCH_LOGIN` (`marfapr`) в эту демо-выборку не входит.
...
4. Картинка (если отмечено) — `⬜️/✅ Превью стрима` (кадр эфира Helix, обновление каждые 30 мин), `⬜️/✅ Видео-превью стрима ⭐` (GIF ~30 с через Create Clip + CloudConvert, обновление каждые 30 мин; входит в Продвинутые опции), `⬜️/✅ Использовать обложку игры` или своё изображение и позиция: в начале или в конце
...
| `TWITCH_CLIPS_REFRESH_TOKEN` | (опц.) refresh-токен Twitch с `clips:edit` — Create Clip для видео-превью |
| `CLOUDCONVERT_API_KEY` | (опц.) CloudConvert — MP4→GIF для видео-превью (файлы не хранятся у нас) |
| `STREAM_PREVIEW_REFRESH_SECONDS` | Интервал обновления превью/GIF на эфире (по умолчанию 1800) |
