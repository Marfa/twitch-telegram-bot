...
При первом `/start` бот предложит выбрать язык (русский / English / українська / italiano), затем покажет приветствие и **главное меню**. При первом запуске (не share) для **ru** (и **uk**) также создаётся welcome-оповещение на случайный Премиум-канал; для **en**/**it** канал `PREMIUM_TWITCH_LOGIN` (`marfapr`) в эту демо-выборку не входит.
...
| ↳ 🔑 Токены авторизации | Одним нажатием отозвать токены Twitch и DonationAlerts |
