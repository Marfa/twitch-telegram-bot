...
| Расписание стримов | **📅 Управление расписанием** в **📦 Прочее**: недельный мастер или **поправить слоты на день** (🧪 бета), **Часовой пояс** (UTC); публикация на Twitch — **Premium** |
| Системные оповещения | Вкл/выкл рассылок об обновлениях, доступности и прочих; падения Twitch (status.twitch.com) |
| Премиум | Триал 7 дней (после окончания — пауза оповещений и уведомление в ЛС); Stars месяц/год/lifetime; à la carte (продвинутый режим, история за 60 дней; корзина / чат стримов — после публичного релиза бет); саб Twitch (`PREMIUM_TWITCH_LOGIN`); разовый **Премиум-канал** для стримеров (`PREMIUM_CHANNEL_STARS`) |
| Партнёрка | Реферальная ссылка, 10% от Stars Premium приглашённых, заявки на вывод (вручную) |
| Админка | Рассылка в фоне (без подвисания) с типом в подписи; отложенная — по UTC+3 (MSK), у пользователей со своим UTC — в их локальное время; статистика после всех волн; тип «обновление бота» обновляет главное меню; DeepL, статистика, выводы, **Демо режим** |
...
**📅 Управление расписанием** в **📦 Прочее** (доступно всем). Участникам беты `schedule-fix-day` (🧪 Бета-режим) предлагается выбор:
...
Без opt-in в бету — старый недельный flow (кнопка **Часовой пояс** на экране подтверждения недели).
