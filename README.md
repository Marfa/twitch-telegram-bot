...
| Продвинутые опции оповещений | Игнор слов, отложенная отправка, заглушка повторов, удаление предыдущих, закрепление до конца стрима, отмена стрима по расписанию |
...
3. **Дополнительно** — чеклист: изображение, очистка названия, игнор ключевых слов ⭐, отложенная отправка ⭐, заглушка повторов ⭐ (начало стрима), удалять предыдущие в канале/группе ⭐, закреплять сообщение до конца стрима ⭐, пользовательзовательские кнопки ⭐/🧪, кнопка чата, 🧪 «Кнопка напоминания» (предстоящий), **«Отмена стрима» ⭐** (предстоящий, последняя), цвет кнопок (по умолчанию / синий / зелёный / красный — появляется при любой включённой кнопке), превью ссылок (если в шаблоне есть URL); у бесплатных ⭐-опции видны, но не включаются; неотмеченные шаги пропускаются; превью тихо выключается при изображении или кнопке чата
