...
| Подписки | Список, вкл/выкл, редактирование всех полей, удаление; **⏸ Приостановить оповещения** на N дней без паузы подписок (🧪 бета); **🧺 Корзина удалённых подписок** — восстановление за 10 дней (30 с Premium) (🧪 бета); **💬 Чат стримов** — Mini App с embed/fallback (🧪 бета) |
...
| Премиум | Триал 7 дней; Stars месяц/год/lifetime; à la carte (продвинутый режим, история за 60 дней; корзина / чат стримов — после публичного релиза бет); саб Twitch (`PREMIUM_TWITCH_LOGIN`); разовый **Премиум-канал** для стримеров (`PREMIUM_CHANNEL_STARS`) |
...
Без opt-in в бету корзина скрыта, удалённые подписки не сохраняются. Покупка à la carte «Корзина…» в Premium появляется после публичного релиза беты (GA).

### Чат стримов Twitch

🧪 Бета-функция `stream-chat` — включается в **⚙️ Настройки → 🧪 Бета-режим**.

После opt-in слева у поля ввода появляется Menu Button **Чат** → Mini App: онлайн из активных подписок, поиск по имени/ссылке, чат стрима (Twitch embed + простой fallback). Бесплатно: чтение + до 20 сообщений в сутки; Premium-функция `stream_chat` / полный план — без лимита.

Без opt-in кнопка меню скрыта. Покупка à la carte «Безлимитный чат…» в Premium появляется после публичного релиза беты (GA).
