...
| Типы оповещений | Начало стрима · смена категории · предстоящий (Twitch schedule) · окончание стрима · Drops (бета) · оповещение по игре |
...
| Follow/Unfollow канала (🧪 бета) | Ежедневный мониторинг Follow/Unfollow своего канала: списки и поиск |
...
**Шаблон сообщения** — в мастере показаны примеры `{username}`, `{game}`, `{name}`. Полный список плейсхолдеров (в т.ч. `started_at`, `viewer_count`, `viewer_avg`, `viewer_peak`, `thumbnail_url`, `tags`, …): [`PUBLIC_BASE_URL/placeholders`](https://bot.themarfa.name/placeholders?lang=ru) (прод: `https://bot.themarfa.name`). **Очистка названия**: при создании — на экране **Дополнительно**; при правке — в меню редактора (те же подписи, что в «Дополнительно»). Включает очистку `{name}` — убираются `@username`, если такой стример есть на Twitch, и команды вида `!команда`. По умолчанию снято.
...
| 📦 Прочее | Follow/Unfollow (бета, Premium), оповещения об ЛС, расписание, что посмотреть (мне повезёт), чат |
...
Данные об играх (случайный подбор, игнор жанров/издателей и т.п.) предоставлены [IGDB.com](https://www.igdb.com).
