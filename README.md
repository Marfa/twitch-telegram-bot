...
**Редактирование подписки** — в том же порядке, что и при создании: шаблон (с чекбоксом **Очистка названия**), картинка, ключевые слова / задержка / повторы / удаление (пункты меню только в **продвинутом режиме**), превью ссылок (скрыто, если есть картинка), напоминания по расписанию (если включали при создании), куда слать. Для **смены категории** при включённом удалении — отдельно «удалять и другие оповещения». В конце меня: **Сменить категорию** (тип оповещения), **Скопировать**, **Скопировать и сменить категорию** (при отмене выбора типа копия не создаётся).
