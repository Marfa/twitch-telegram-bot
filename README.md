...
| **Режим отпуска** | Календарь начала/конца → выход автоматически или нет → Vacation Mode на Twitch (**Premium**); оповещения «Предстоящий стрим» в период отпуска не уходят |
| **Часовой пояс** | Указать UTC (`UTC+3`, `UTC-5`, …); сохраняется для публикации на Twitch |

**Недельный мастер:**

1. Для каждого дня: слоты текстом (`15:30 Disponia` — несколько строк сразу)
